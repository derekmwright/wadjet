...
- **Table functions** — `read_json()`, `read_csv()`, `read_parquet()` query local files and HTTP URLs directly from SQL, with glob patterns and named parameters; `generate_series()` and `unnest()` generate rows. A table function in `FROM` is a relation: it publishes a column list *before the query runs* — a Parquet footer, a CSV header, a sampled JSON batch — so a reference to a column it does not publish is `42703` naming the column, `f.*` expands, and its columns carry their type into aggregates and joins.
