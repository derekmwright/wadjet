...
// RetireObjects deletes only objects proven unreferenced by EVERY live table
// (#896). Mark paths FIRST: in-flight registrations yield RetireUnproven,
// and new registrations of marked paths fail with ErrPathRetiring.
// Then check all live manifests: references yield RetireReferenced.
// Finally preserve objects modified since scheduling; catalog-read failures
// yield RetireUnproven for all paths. Doubt preserves bytes.
// The mark excludes registrations through this Catalog, not another process;
// closing that gap needs a catalog-side lease. The queue is process-local.
// See docs/internals/catalog-object-retirement-interlock.md for the design.
