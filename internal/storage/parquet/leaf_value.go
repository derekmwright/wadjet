...
// int32LeafValue/int64LeafValue normalize admitted integer boxes and enforce
// range BEFORE Go narrowing, raising SQLSTATE 22003 via sqlerr.
// Check floating values before conversion: NaN/infinities/out-of-range values
// cannot safely become integers. Fractional floats are REFUSED, not rounded.
// Assignment rounding belongs to DML's cast (float half-even, numeric half-away);
// a fraction at the leaf means that cast never happened (ADR-0018).
// Other boxes must refuse rather than fall through to a stored zero.
// See docs/internals/parquet-integer-leaf-value-checks.md for the design.
