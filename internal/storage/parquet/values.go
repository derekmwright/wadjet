...
// typedValues is the shared zero-copy fixed-width view safety check.
// Require the requested physical type and enough bytes for ALL v.count
// width-byte elements; nonpositive count/width returns nil.
// Never expose a wider unsafe.Slice than its backing array, or truncate a
// short buffer into fewer values that callers might treat as absent.
// Return nil on refusal; read sites must report a column-named error.
// Decoder length checks and retypeFromCatalog are earlier guards.
// See docs/internals/parquet-fixed-width-view-safety.md for the design.
