...
// MaxPageValues bounds header counts before any body-driven allocation.
// 2^24 caps a header-provoked int32 allocation at 64 MiB per column;
// column fan-out can allocate concurrently. The writer obeys the same ceiling.
// This is policy headroom, not an exact body bound: ColumnPageReader.chargeRows
// separately limits flat-leaf chunks to the row group's exact row count.
// See docs/internals/parquet-page-value-policy-ceiling.md for the design.
const MaxPageValues = 1 << 24
