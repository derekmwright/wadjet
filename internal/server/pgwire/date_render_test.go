...
func TestDateBinaryFormat(t *testing.T) {
	...
	for _, tc := range cases {
		...
		if _, err := c.sendResultRows(context.Background(), []string{"d"}, nil, boxedRows([]string{"d"}, rows), []int16{1}, dateMetas(), nil); err != nil {
...
func TestDateTextFormatUnchanged(t *testing.T) {
	...
	if _, err := c.sendResultRows(context.Background(), []string{"d"}, nil, boxedRows([]string{"d"}, rows), []int16{0}, dateMetas(), nil); err != nil {
		t.Fatalf("sendResultRows: %v", err)
	}
