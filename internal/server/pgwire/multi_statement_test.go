...
func TestSimpleProtocolRunsEachStatementAndEndsWithOneReadyForQuery(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "trailing semicolon",
			sql:  "DELETE FROM m711 WHERE id = 1;",
			want: "CZ"},
		// A trailing COMMENT is not a second statement: one CommandComplete,
		// not an ErrorResponse (review B1).
		{name: "trailing line comment",
			sql:  "DELETE FROM m711 WHERE id = 1; -- audit note",
			want: "CZ"},
		{name: "trailing block comment",
			sql:  "DELETE FROM m711 WHERE id = 1; /* audit */",
			want: "CZ"},
		{name: "leading comment",
			sql:  "-- lead\nDELETE FROM m711 WHERE id = 1",
			want: "CZ"},
		{name: "a comment-only piece between two statements",
			sql:  "DELETE FROM m711 WHERE id = 1; /* mid */ ; DELETE FROM m711 WHERE id = 2",
			want: "CCZ"},
		{name: "a comment-only string is one EmptyQueryResponse",
			sql:  "-- nothing to run",
			want: "IZ"},
		{name: "a block-comment-only string is one EmptyQueryResponse",
			sql:  "/* nothing to run */",
			want: "IZ"},
...
func TestSimpleProtocolMultiStatementTagsAreOnePerStatement(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "a trailing comment does not add a tag", want: []string{"DELETE 1"},
			sql: "DELETE FROM m711 WHERE id = 1; -- audit note"},
		{name: "a comment-only piece between two statements", want: []string{"DELETE 1", "DELETE 1"},
			sql: "DELETE FROM m711 WHERE id = 1; /* mid */ ; DELETE FROM m711 WHERE id = 2"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			srv := startTestServer(t, multiStmtDB(t))
