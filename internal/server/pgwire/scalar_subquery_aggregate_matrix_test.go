...
// here. The 24 wide DECIMAL value divergences were #1037 — a wide numeric
// literal reaching the CAST as the float64 box compileLit built — and their
// pins are GONE: the twelve `numeric/*` entries carry PostgreSQL's own digits
...
func TestScalarSubqueryAggregateMatrix(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"numeric/SUM/plain", "SELECT SUM((SELECT CAST(9007199254740993.25 AS DECIMAL(30,2)))) AS v FROM users", 1700, 1700, "108086391056891919/4", "108086391056891919/4"},
		{"numeric/SUM/grouped", "SELECT SUM((SELECT CAST(9007199254740993.25 AS DECIMAL(30,2)))) AS v FROM users GROUP BY id ORDER BY id", 1700, 1700, "36028797018963973/4;36028797018963973/4;36028797018963973/4", "36028797018963973/4;36028797018963973/4;36028797018963973/4"},
		{"numeric/SUM/window", "SELECT SUM((SELECT CAST(9007199254740993.25 AS DECIMAL(30,2)))) OVER () AS v FROM users", 1700, 1700, "108086391056891919/4;108086391056891919/4;108086391056891919/4", "108086391056891919/4;108086391056891919/4;108086391056891919/4"},
		{"numeric/AVG/plain", "SELECT AVG((SELECT CAST(9007199254740993.25 AS DECIMAL(30,2)))) AS v FROM users", 1700, 1700, "36028797018963973/4", "36028797018963973/4"},
		{"numeric/AVG/grouped", "SELECT AVG((SELECT CAST(9007199254740993.25 AS DECIMAL(30,2)))) AS v FROM users GROUP BY id ORDER BY id", 1700, 1700, "36028797018963973/4;36028797018963973/4;36028797018963973/4", "36028797018963973/4;36028797018963973/4;36028797018963973/4"},
		{"numeric/AVG/window", "SELECT AVG((SELECT CAST(9007199254740993.25 AS DECIMAL(30,2)))) OVER () AS v FROM users", 1700, 1700, "36028797018963973/4;36028797018963973/4;36028797018963973/4", "36028797018963973/4;36028797018963973/4;36028797018963973/4"},
		{"numeric/MIN/plain", "SELECT MIN((SELECT CAST(9007199254740993.25 AS DECIMAL(30,2)))) AS v FROM users", 1700, 1700, "36028797018963973/4", "36028797018963973/4"},
		{"numeric/MIN/grouped", "SELECT MIN((SELECT CAST(9007199254740993.25 AS DECIMAL(30,2)))) AS v FROM users GROUP BY id ORDER BY id", 1700, 1700, "36028797018963973/4;36028797018963973/4;36028797018963973/4", "36028797018963973/4;36028797018963973/4;36028797018963973/4"},
		{"numeric/MIN/window", "SELECT MIN((SELECT CAST(9007199254740993.25 AS DECIMAL(30,2)))) OVER () AS v FROM users", 1700, 1700, "36028797018963973/4;36028797018963973/4;36028797018963973/4", "36028797018963973/4;36028797018963973/4;36028797018963973/4"},
		{"numeric/MAX/plain", "SELECT MAX((SELECT CAST(9007199254740993.25 AS DECIMAL(30,2)))) AS v FROM users", 1700, 1700, "36028797018963973/4", "36028797018963973/4"},
		{"numeric/MAX/grouped", "SELECT MAX((SELECT CAST(9007199254740993.25 AS DECIMAL(30,2)))) AS v FROM users GROUP BY id ORDER BY id", 1700, 1700, "36028797018963973/4;36028797018963973/4;36028797018963973/4", "36028797018963973/4;36028797018963973/4;36028797018963973/4"},
		{"numeric/MAX/window", "SELECT MAX((SELECT CAST(9007199254740993.25 AS DECIMAL(30,2)))) OVER () AS v FROM users", 1700, 1700, "36028797018963973/4;36028797018963973/4;36028797018963973/4", "36028797018963973/4;36028797018963973/4;36028797018963973/4"},
		...
		{"max/MIN/plain", "SELECT MIN((SELECT MAX(id) FROM users)) AS v FROM users", 23, 23, "3", "3"},
		{"max/MIN/grouped", "SELECT MIN((SELECT MAX(id) FROM users)) AS v FROM users GROUP BY id ORDER BY id", 23, 23, "3;3;3", "3;3;3"},
		{"max/MIN/window", "SELECT MIN((SELECT MAX(id) FROM users)) OVER () AS v FROM users", 23, 23, "3;3;3", "3;3;3"},
		{"max/MAX/plain", "SELECT MAX((SELECT MAX(id) FROM users)) AS v FROM users", 23, 23, "3", "3"},
		{"max/MAX/grouped", "SELECT MAX((SELECT MAX(id) FROM users)) AS v FROM users GROUP BY id ORDER BY id", 23, 23, "3;3;3", "3;3;3"},
		{"max/MAX/window", "SELECT MAX((SELECT MAX(id) FROM users)) OVER () AS v FROM users", 23, 23, "3;3;3", "3;3;3"},
