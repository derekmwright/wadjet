...
// reclaimLease resolves a failed refresh by reading the current lease holder (#559).
// If the key expired (5s TTL, 2s refresh), race standbys immediately with atomic Create;
// exactly one instance wins, preventing split-brain.
// If another instance holds it, stand down.
// If we hold it at an unrecorded revision, adopt that revision: the update may
// have landed without its acknowledgement.
// Any other error leaves ownership unknown and requires standing down.
// See docs/internals/leader-lease-refresh-reconciliation.md for the design.
