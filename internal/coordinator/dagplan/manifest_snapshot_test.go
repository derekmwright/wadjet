...
// ExecuteSQL/SubmitSQL wiring. Without the pin (plain physical.NewPlanner, no
...
func TestManifestSnapshotPinsReadsAcrossAStatement(t *testing.T) {
	...
	ctx = WithManifestSnapshot(ctx, physical.NewManifestSnapshot())

	logicalPlan := selfJoinLogicalPlan(t)
	scanAnnotator := func(plan *logical.Node) {
		physical.NewPlannerForContext(ctx, cat).AnnotateScanColumns(ctx, plan)
	}
	...
	fastPlanner := NewStagePlanner(physical.NewPlannerForContext(ctx, cat))
	...
	planner := NewStagePlanner(physical.NewPlannerForContext(ctx, cat))
...
func TestManifestSnapshotUnpinnedReadsRepeatedly(t *testing.T) {
	...
	logicalPlan := selfJoinLogicalPlan(t)
	scanAnnotator := func(plan *logical.Node) {
		physical.NewPlanner(cat).AnnotateScanColumns(ctx, plan) // fresh Planner, fresh (unshared) snapshot
	}
	scanAnnotator(logicalPlan)
	logicalPlan = logical.Optimize(logicalPlan, scanAnnotator)
...
func TestManifestSnapshotIgnoresAConcurrentWriteMidStatement(t *testing.T) {
	...
	ctx2 := WithManifestSnapshot(context.Background(), physical.NewManifestSnapshot())
	logicalPlan2 := selfJoinLogicalPlan(t)
	physical.NewPlannerForContext(ctx2, cat).AnnotateScanColumns(ctx2, logicalPlan2)
	logicalPlan2 = logical.Optimize(logicalPlan2, func(plan *logical.Node) {
		physical.NewPlannerForContext(ctx2, cat).AnnotateScanColumns(ctx2, plan)
	})
	planner2 := NewStagePlanner(physical.NewPlannerForContext(ctx2, cat))
...
func TestManifestSnapshotPinsTheMetadataFoldReads(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			ann2 := func(plan *logical.Node) { physical.NewPlanner(cat2).AnnotateScanColumns(ctx2, plan) }
			ann2(logicalPlan2)
			logicalPlan2 = logical.Optimize(logicalPlan2, ann2)
			p2 := NewStagePlanner(physical.NewPlanner(cat2))
			p2.WorkerCount = 2
