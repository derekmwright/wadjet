...
// fuseStageChains fuses P hash_join with its sole consumer C hash/broadcast_join
// when C reads P only as probe and both are HashPartitioned with the same count.
// Neither may carry Exchange/SortKeys/Limit/GroupByCols/probe-split/build-cache/
// merge-tree fields. P's fused/chained joins remain; C's fused probes precede C.
// Keep P's ID/type/keys/primary build; adopt C's exact distribution/output estimate
// and auxiliary build deps. Forbid dep aliasing except shared primary builds
// (hash-partitioned only); preserve duplicate dep entry and upstream-ID lookup.
// Run to fixpoint after final distributions, elision, subsume and agg-over-exchange.
// See docs/internals/stage-chain-fusion.md for the design.
...
func fuseOneChainLink(stages []Stage) ([]Stage, bool) {
	...
	if StageFusionAgg.Load() {
		for pi := range stages {
			...
			p.GroupByResolve = append([]physical.GroupKeyResolution(nil), c.GroupByResolve...)
			...
			p.GroupByDecimal = c.GroupByDecimal
