...
func TestResolveOutputRenameSource(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			if got := physical.ResolveOutputRenameSource(tc.in, tc.child); got != tc.want {
...
func TestScopePreservingWrapperMatchesTheRenameWalk(t *testing.T) {
	...
	for id, want := range table {
		...
		t.Run(id.String(), func(t *testing.T) {
			...
			if got := physical.ScopePreservingWrapper(n); got != want.wrapper {
...
func nodeTypeConstNames(t *testing.T) []string {
	t.Helper()
	files, err := filepath.Glob(filepath.Join("..", "..", "planner", "logical", "*.go"))
	if err != nil || len(files) == 0 {
		t.Fatalf("globbing ../../planner/logical/*.go: %v (%d files) — this test reads the NodeType "+
	...
	t.Fatal("no `NodeType` iota const block found under ../../planner/logical — this test reads it to " +
