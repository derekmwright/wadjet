...
func TestOwnsKeyRecognizesADerivedTablesOutputColumn(t *testing.T) {
	...
	y := physical.SubtreeNamingOf(derived("y", "n_nationkey", "b"))
	z := physical.SubtreeNamingOf(derived("z", "r_regionkey", "c"))
	...
	physical.AssignJoinKeySides(left, right, y, z)
...
func TestOwnsKeyStillRefusesTheOtherSelfJoinCopy(t *testing.T) {
	n1 := physical.SubtreeNamingOf(&logical.Node{Type: logical.NodeScan, TableName: "n", TableAlias: "n1",
