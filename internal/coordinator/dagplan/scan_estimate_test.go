...
func TestEstimatePlanScanBytes(t *testing.T) {
	...
	buildOptimized := func(sql string) *logical.Node {
		...
		annotate := func(p *logical.Node) { physical.NewPlanner(cat).AnnotateScanColumns(ctx, p) }
		annotate(plan)
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			got, ok := NewStagePlanner(physical.NewPlanner(cat)).EstimatePlanScanBytes(ctx, plan)
			if ok != tc.ok {
	...
	if _, ok := NewStagePlanner(physical.NewPlanner(cat)).EstimatePlanScanBytes(ctx, unknown); ok {
	...
	if _, ok := NewStagePlanner(physical.NewPlanner(cat)).EstimatePlanScanBytes(ctx, sub); ok {
