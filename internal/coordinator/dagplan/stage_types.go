...
type Stage struct {
	...
	GroupByResolve []physical.GroupKeyResolution
	...
	WindowKeyExprs []physical.ProjectExprSpec
	...
	ProjectExprs []physical.ProjectExprSpec
	...
	SecurityProjectExprs []physical.ProjectExprSpec
...
type OutputRename struct {
	...
	// from the zero value the way physical.ProjectExprSpec.TypeKnown does — TypeBool
	...
	// sort key and for every physical.ProjectExprSpec. Without it the gather built
...
type UnionArm struct {
	Projections []physical.ProjectExprSpec
	...
	//
	// It is a separate list rather than a field on physical.ProjectExprSpec because
	...
	DecimalCoercions []physical.DecimalCoercion
}
...
type AggSpec struct {
	...
	// physical.GroupKeyResolution.Alias/Def one consumer over: which spelling the
...
type SortKeySpec struct {
	...
	// no longer read the question off it. physical.Planner-only; the wire carries
	...
	//
	// physical.Planner-only: what reaches the wire is the SlotPos it settles.
...
// PreComputedAggregateMeta travels on Stage to tell task creation
