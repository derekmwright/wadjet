...
func TestDimensionCascadeNegatives(t *testing.T) {
	...
	for name, mutate := range cases {
		stages := cascadeFixture()
		mutate(stages)
		NewStagePlanner(physical.NewPlanner(cat)).markDimensionCascade(ctx, stages)
		for i := range stages {
...
func TestDimensionCascadeKillSwitch(t *testing.T) {
	...
	cat, ctx := setupTPCHCatalog(t)
	stages := cascadeFixture()
	NewStagePlanner(physical.NewPlanner(cat)).markDimensionCascade(context.Background(), stages)
	_ = ctx
	for i := range stages {
