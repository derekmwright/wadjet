...
func TestHasFilterOrPartition_NilNode(t *testing.T) {
	if physical.HasFilterOrPartition(nil) {
...
func TestHasFilterOrPartition_DeepFilter(t *testing.T) {
	...
	if !physical.HasFilterOrPartition(project) {
...
func TestFormatBytes(t *testing.T) {
	...
	for _, tt := range tests {
		got := physical.FormatBytes(tt.input)
		if got != tt.want {
...
func TestMapJoinType(t *testing.T) {
	...
	for _, tt := range tests {
		got := physical.MapJoinType(tt.input)
		if got != tt.want {
...
func TestMapExecJoinType(t *testing.T) {
	...
	for _, tt := range tests {
		got := physical.MapExecJoinType(tt.input)
		if got != tt.want {
...
func TestParseJoinKeys(t *testing.T) {
	...
	for _, tt := range tests {
		left, right, residual := physical.ParseJoinKeys(tt.cond)
...
func TestParseJoinKeysRefusesUnrepresentable(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, _, residual := physical.ParseJoinKeys(tt.cond)
			if len(residual) == 0 {
				left, right, _ := physical.ParseJoinKeys(tt.cond)
...
func TestMatchesPartitionFilter(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := physical.MatchesPartitionFilter(tt.partVals, tt.filter)
			if got != tt.want {
...
func TestEvalFilterTyped(t *testing.T) {
	...
	bv.Int32Data[0] = 20
	bv.Int32Data[1] = 20
	if !physical.EvalFilterTyped(pv, bv, 0, 0, opNE) {
		t.Error("10 != 20 should be true")
	}
	if physical.EvalFilterTyped(pv, bv, 1, 1, opNE) {
		t.Error("20 != 20 should be false")
	}
	if !physical.EvalFilterTyped(pv, bv, 0, 0, opLT) {
		t.Error("10 < 20 should be true")
	}
	if !physical.EvalFilterTyped(pv, bv, 1, 1, opEQ) {
	...
	if !physical.EvalFilterTyped(sv, tv, 0, 0, opNE) {
		t.Error("a != b should be true")
	}
	if physical.EvalFilterTyped(sv, tv, 1, 1, opNE) {
		t.Error("b != b should be false")
	}
	if !physical.EvalFilterTyped(sv, tv, 0, 0, opLT) {
		t.Error("a < b should be true")
	}
}
...
func TestDecimalFromBytes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := physical.DecimalFromBytes(tt.input)
...
func TestBuildTableFunctionSource_CSV_SepArg(t *testing.T) {
	source, err := physical.BuildTableFunctionSource("read_csv", []string{"/tmp/test.csv"}, map[string]string{"sep": "|"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	csvSrc, ok := source.(*physical.CsvTableFuncSource)
	if !ok {
...
func TestBuildTableFunctionSource_ReadJSONAuto(t *testing.T) {
	source, err := physical.BuildTableFunctionSource("read_json_auto", []string{"/tmp/data.json"}, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if _, ok := source.(*physical.JsonTableFuncSource); !ok {
...
func TestBuildTableFunctionSource_ReadCSVAuto(t *testing.T) {
	source, err := physical.BuildTableFunctionSource("read_csv_auto", []string{"/tmp/data.csv"}, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if _, ok := source.(*physical.CsvTableFuncSource); !ok {
...
func TestResolveNullsLast(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := physical.ResolveNullsLast(tc.ob); got != tc.want {
...
func TestExtractFilterBuildColumns(t *testing.T) {
	// Empty
	cols := physical.ExtractFilterBuildColumns("")
	if cols != nil {
		t.Errorf("expected nil for empty, got %v", cols)
	}

	// Simple equality
	cols = physical.ExtractFilterBuildColumns("e.id = u.id")
	if len(cols) != 1 {
	...
	cols = physical.ExtractFilterBuildColumns("a.x = b.x AND a.y = b.y")
	if len(cols) != 2 {
		t.Fatalf("expected 2 columns, got %d: %v", len(cols), cols)
	}

	// No operator matches
	cols = physical.ExtractFilterBuildColumns("unknown stuff")
	if len(cols) != 0 {
...
func TestCleanExpr(t *testing.T) {
	if physical.CleanExpr("t.col") != "col" {
		t.Error("expected 'col' for 't.col'")
	}
	if physical.CleanExpr("col") != "col" {
...
func TestCleanExprLeavesExpressionsAlone(t *testing.T) {
	for _, tc := range []struct{ in, want string }{
		...
		if got := physical.CleanExpr(tc.in); got != tc.want {
