...
func TestSetOpWidenLadder(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		got, ok := physical.SetOpWiden(tc.a, tc.b)
...
func TestSetOpDecimalTarget(t *testing.T) {
	...
	intArm := func(t parquet.TypeID) physical.SetOpColType {
		return physical.SetOpColType{Typ: t, Known: true}
	}
	for _, tc := range []struct {
		name   string
		arms   []physical.SetOpColType
		wantP  int
		wantS  int
		wantOk bool
	}{
		{"same_type", []physical.SetOpColType{dec(9, 2), dec(9, 2)}, 9, 2, true},
		// The issue's own pair: 7 integer digits and 14, so 14 + scale 4.
		{"the_issue", []physical.SetOpColType{dec(9, 2), dec(18, 4)}, 18, 4, true},
		{"reversed", []physical.SetOpColType{dec(18, 4), dec(9, 2)}, 18, 4, true},
		{"three_arms", []physical.SetOpColType{dec(9, 2), dec(18, 4), dec(38, 10)}, 38, 10, true},
		// Where max(precision) is WRONG: 16 integer digits at scale 4 needs
		// 20, and max(precision) would say 18.
		{"integer_part_beats_max_precision", []physical.SetOpColType{dec(18, 2), dec(9, 4)}, 20, 4, true},
		// Capped at the Int128 carrier's width; a value that does not fit is
		// then an error at coercion, never a wrapped number.
		{"capped_at_38", []physical.SetOpColType{dec(38, 2), dec(38, 10)}, 38, 10, true},
		// An integer arm contributes its whole range's digits.
		{"with_int32", []physical.SetOpColType{dec(9, 2), intArm(parquet.TypeInt32)}, 12, 2, true},
		{"with_int64", []physical.SetOpColType{dec(9, 2), intArm(parquet.TypeInt64)}, 21, 2, true},
		{"only_ints", []physical.SetOpColType{intArm(parquet.TypeInt32), intArm(parquet.TypeInt64)}, 19, 0, true},
		// An arm whose (p,s) nothing resolved: no target, which
		// reconcileSetOpArmTypes turns into a plan-time refusal (#551).
		{"unresolved_arm", []physical.SetOpColType{dec(9, 2),
			{Typ: parquet.TypeDecimal, Known: true}}, 0, 0, false},
		{"non_numeric_arm", []physical.SetOpColType{dec(9, 2),
			{Typ: parquet.TypeString, Known: true}}, 0, 0, false},
		{"no_arms", nil, 0, 0, false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			got, ok := physical.SetOpDecimalTarget(tc.arms)
			if ok != tc.wantOk {
...
func TestSetOpDecimalTargetNeverNarrowsAnArm(t *testing.T) {
	for _, arms := range [][]physical.SetOpColType{
		...
		want, ok := physical.SetOpDecimalTarget(arms)
		if !ok {
