...
// TypeKnown-forgetter sweep: a corpus of one plan per physical.ProjectExprSpec
...
func TestProjectExprSpecsDeclareTypeKnownForBooleanOutputs(t *testing.T) {
	...
	for _, tc := range []struct {
		name string
		sql  string
		// wantCol is the physical.ProjectExprSpec name the site under test
...
// assertBoolSpecTypeKnown finds the physical.ProjectExprSpec named col across every
...
func assertBoolSpecTypeKnown(t *testing.T, stages []Stage, col string) {
	t.Helper()
	var found *physical.ProjectExprSpec
	...
	if found == nil {
		t.Fatalf("no physical.ProjectExprSpec named %q across any stage — this shape no longer exercises "+
...
// allProjectExprSpecs collects every physical.ProjectExprSpec a stage list carries,
...
// per-arm Projections.
func allProjectExprSpecs(stages []Stage) []physical.ProjectExprSpec {
	var specs []physical.ProjectExprSpec
