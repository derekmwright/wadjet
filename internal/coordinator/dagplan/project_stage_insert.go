...
func insertProjectStageAbove(stages []Stage, targetIdx int, specs []physical.ProjectExprSpec) []Stage {
...
func projectionNeedsItsOwnStage(s *Stage, specs []physical.ProjectExprSpec) bool {
...
func aliasedSpecsFor(proj []logical.Projection, specs []physical.ProjectExprSpec,
	slotPassThrough []bool) []physical.ProjectExprSpec {
	out := make([]physical.ProjectExprSpec, len(specs))
...
func projectionCoversFilters(specs []physical.ProjectExprSpec, filters []string) bool {
	if len(filters) == 0 {
		return true
	}
...
func specsResolveAgainstStageInput(stages []Stage, targetIdx int, specs []physical.ProjectExprSpec) bool {
...
func specsResolveAgainstStageOutput(stages []Stage, producerIdx int, specs []physical.ProjectExprSpec) bool {
...
func specsResolveAgainst(emitted map[string]string, specs []physical.ProjectExprSpec) bool {
	for _, sp := range specs {
		if sp.Expr == "" {
			continue
		}
...
func respellSpecsOverProducerOutput(stages []Stage, producerIdx int, specs []physical.ProjectExprSpec) ([]physical.ProjectExprSpec, bool) {
	if producerIdx < 0 || producerIdx >= len(stages) {
		return specs, true
	}
	idx := make(map[string]int, len(stages))
...
func orderingSurvivesAProjectStage(stages []Stage, producerIdx int, specs []physical.ProjectExprSpec) bool {
	if producerIdx < 0 || producerIdx >= len(stages) {
		return true
	}
