...
func absorbComputedSubqueryProjection(child *logical.Node, childStages []Stage, requireEnclosing bool) bool {
	...
	var computed []physical.ProjectExprSpec
	...
	for _, pr := range proj.Projections {
		...
		spec := physical.ProjectExprSpec{Expr: expr, Name: strings.ToLower(pr.Alias)}
	...
	specs := make([]physical.ProjectExprSpec, 0, len(cols)+len(computed))
	for _, c := range cols {
		specs = append(specs, physical.ProjectExprSpec{Expr: c, Name: c})
	}
...
// behaviour.
func absorbWindowArmProjection(childStages []Stage, computed []physical.ProjectExprSpec,
	...
	sort.Strings(names)
	specs := make([]physical.ProjectExprSpec, 0, len(names)+len(computed))
	for _, n := range names {
		specs = append(specs, physical.ProjectExprSpec{Expr: n, Name: n})
	}
	specs = append(specs, computed...)
	target.ProjectExprs = specs
}
...
func absorbJoinArmProjection(childStages []Stage, computed []physical.ProjectExprSpec,
...
func windowArmColDecls(win *logical.Node) physical.ColDecls {
	...
	return physical.ColDecls{Types: types, Fields: base.Fields, Dec: dec}
}
