...
func TestAliasedSortKeyWithoutAggregateResolves(t *testing.T) {
	...
	tests := []struct {
		...
		wantProjected []physical.ProjectExprSpec
	}{
		{
			// #316 verbatim.
			...
			want:          []string{"p"},
			wantProjected: []physical.ProjectExprSpec{{Expr: "o_orderpriority", Name: "p"}},
		},
		{
		...
		},
		{
			...
			want:          []string{"nm"},
			wantProjected: []physical.ProjectExprSpec{{Expr: "n_name", Name: "nm"}},
		},
		{
