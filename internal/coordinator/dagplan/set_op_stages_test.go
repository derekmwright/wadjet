...
func planSetOpStages(t *testing.T, cat *catalog.Catalog, ctx context.Context, sql string) ([]Stage, error) {
	...
	annotate := func(n *logical.Node) { physical.NewPlanner(cat).AnnotateScanColumns(ctx, n) }
	annotate(plan)
...
// (physical.DecimalCoercion), never by a CAST — the cast evaluator's DECIMAL
...
func TestAlignSetOpRows(t *testing.T) {
	...
	got := physical.AlignSetOpRows(left, right, rows)
	if len(got) != 2 {
