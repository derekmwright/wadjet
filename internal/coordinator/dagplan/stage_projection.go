...
func (p *StagePlanner) attachScanSelectProjections(root *logical.Node, stages []Stage) []Stage {
	...
	for j, it := range proj {
		...
		if it.ASTExpr != nil && (!p.PlanContext.IsSimpleColRefForRename(it.ASTExpr) || p.PlanContext.AstIsFieldPath(it.ASTExpr, colTypes)) {
			if p.PlanContext.ReferencesSyntheticAgg(it.ASTExpr) || referencesSyntheticWindow(it.ASTExpr) {
				...
				specs = append(specs, physical.ProjectExprSpec{Expr: slots[0], Name: slots[0]})
				continue
			}
			hasExpr = true
	...
	for _, slot := range extraSlots {
		...
		if !dup {
			specs = append(specs, physical.ProjectExprSpec{Expr: slot, Name: slot})
		}
	}
	...
	for j := range specs {
		...
		if def, owner := derivedAliasDefinition(specs[j].Name, renameChild); def != nil && owner != nil && len(owner.Children) == 1 {
			...
			if !published {
				...
				specs[j].ElementType = d.ElementType
				specs[j].TypeKnown = true
				hasExpr = true
				anyNestedRename = true
				continue
	...
	for i := range stages {
		...
		// projected set).
		aliased := make([]physical.ProjectExprSpec, len(specs))
...
func repointGatherRenames(gather *Stage, aliased []physical.ProjectExprSpec) {
...
func anyRenamed(proj []logical.Projection, specs []physical.ProjectExprSpec, slotPassThrough []bool) bool {
	for j, p := range proj {
...
// source column.
func sortKeysNeedAlias(sortKeys []SortKeySpec, specs, aliased []physical.ProjectExprSpec) bool {
