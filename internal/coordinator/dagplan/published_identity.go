...
func uniqueOutputName(specs []physical.ProjectExprSpec, term string) (string, bool) {
	name, hits := "", 0
	for _, p := range specs {
...
// onto the producer.
func selectListAbove(stages []Stage, s *Stage) []physical.ProjectExprSpec {
...
func uniqueOutputSource(specs []physical.ProjectExprSpec, term string) (string, bool) {
	src, hits := "", 0
	for _, p := range specs {
		if p.Name == "" || !strings.EqualFold(p.Name, term) {
			continue
