...
// Scan-task affinity assigns each file a canonical active-worker owner by
// rendezvous hashing; fan-outs group by owner and set AffinityWorkerID.
// The scheduler prefers, never requires, that placement (docs/design/scan-affinity.md).
// Base-table caches are per worker. A non-owner miss fetches the owner's NVMe
// copy over the peer wire and populates locally, keeping S3 first touches
// cluster-wide rather than per worker.
// Both halves default on: WADJET_SCAN_AFFINITY=0 disables placement affinity;
// WADJET_BASE_PEER_TIER=0 disables the peer tier.
// See docs/internals/scan-owner-affinity-and-peer-cache.md for the design.
