...
var o2Pin = map[string]map[string]string{
	...
	// #1019's four cells — a correlated lateral's own bound applied to the
	// whole inner relation rather than per outer row — were PINNED here with
	// PostgreSQL's answer beside them and are DELETED: the bound now travels
	// with the correlation key as a per-key top-N
	// (logical.lateralBoundPerOuterRow), and all four assert PostgreSQL's row
	// set from o2PostgresAnswers. Their qualified-star siblings are out of
	// o2Refuses for the same reason.
