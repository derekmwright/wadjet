...
func brArmCells() []brArmCell {
	return []brArmCell{
		...
		// --- #1216 item 1: a CASE's WHEN is a truth context everywhere -----
		{name: "1216/caseWhenUnderComparisonInWhere",
			sql:   "SELECT id FROM lat_ord WHERE CASE WHEN upper(customer) THEN 1 ELSE 0 END = 1",
			state: "42804", msg: "argument of CASE/WHEN must be type boolean, not type text"},
		{name: "1216/caseWhenInJoinOn",
			sql:   "SELECT o.id FROM lat_ord o JOIN lat_item i ON CASE WHEN i.product THEN 1 END = o.id",
			state: "42804", msg: "argument of CASE/WHEN must be type boolean, not type text"},
		{name: "1216/caseWhenOverAggregateInHaving",
			sql:   "SELECT id FROM lat_ord GROUP BY id HAVING CASE WHEN MAX(customer) THEN 1 END = 1",
			state: "42804", msg: "argument of CASE/WHEN must be type boolean, not type text"},
		{name: "1216/caseWhenInOrderBy",
			sql:   "SELECT id FROM lat_ord ORDER BY CASE WHEN customer THEN 1 END",
			state: "42804", msg: "argument of CASE/WHEN must be type boolean, not type text"},
		{name: "1216ok/caseWhenPredicate",
			sql: "SELECT id FROM lat_ord WHERE CASE WHEN id > 1 THEN 1 ELSE 0 END = 1", want: "rows=2 2 | 3"},
		{name: "1216ok/simpleCase",
			sql: "SELECT id FROM lat_ord WHERE CASE customer WHEN 'Bob' THEN 1 ELSE 0 END = 1", want: "rows=1 2"},
		...
		{name: "1060/caseTwoRowShapes",
			...
			state: "42846", msg: "CASE/WHEN could not convert type record("},
		{name: "1060/coalesceTwoRowShapes",
			sql:   "SELECT COALESCE(c_row, c_rownest) AS v FROM typemx_nested WHERE id < 3",
			state: "42846", msg: "COALESCE could not convert type record("},
		...
		{name: "1065/unionAllTwoRowShapes",
			...
			state: "42846", msg: "UNION could not convert type record("},
		...
		{name: "1065/intersectTwoRowShapes",
			...
			state: "42846", msg: "INTERSECT could not convert type record("},
		{name: "1065/exceptTwoRowShapes",
			...
			state: "42846", msg: "EXCEPT could not convert type record("},
...
func brAggregateCells() []brArmCell {
	...
	out = append(out,
		...
		brArmCell{name: "aggState/percentileContOverText", sql: "SELECT percentile_cont(0.5, c_str) AS v FROM typemx WHERE id < 20",
			state: "42883", msg: "function percentile_cont(numeric, text) does not exist"},
		brArmCell{name: "aggState/percentileDiscOverText", sql: "SELECT percentile_disc(0.5, c_str) AS v FROM typemx WHERE id < 20",
			state: "42809", msg: "WITHIN GROUP is required for ordered-set aggregate percentile_disc"},
		brArmCell{name: "aggOk/percentileContOverNumber", sql: "SELECT percentile_cont(0.5, c_i32) AS v FROM typemx WHERE id < 20",
			want: "rows=1 28.5"},
		brArmCell{name: "aggOk/percentileDiscOverNumber", sql: "SELECT percentile_disc(0.5, c_i32) AS v FROM typemx WHERE id < 20",
			want: "rows=1 27"},
