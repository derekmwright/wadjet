...
// runRefusedLocal runs a query for which the DAG produced no plan (#359, #466).
// This is a route: every local failure is the client's outcome, never a retry on another engine.
// There is no scan-byte threshold gate; refused plans may be unestimable.
// The same pipeline memory budget (breakers spill) and collect result budget apply,
// derived from the fast-path threshold or its default when fast path is disabled.
// Exceeding them FAILS with the budget error; larger results require --local-fastpath-bytes.
// what names the construct in messages; counter is the per-class routing counter
// that distributed suites must assert beside rows to prove engagement.
// See docs/internals/refused-dag-local-execution-contract.md for the design.
