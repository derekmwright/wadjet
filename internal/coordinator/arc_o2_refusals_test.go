...
var o2Refuses = map[string]string{
	...
	// APPLIED PER OUTER ROW declined here until #1019 closed: the body's ROW
	// COUNT is now the one the query wrote, so the relation is statable and
	// the two entries are DELETED. Where the per-key rewrite DECLINES — an
	// unreadable bound, a set-operation or DISTINCT body — the mark is still
	// set and this consumer still declines.
}
