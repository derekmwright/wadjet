...
func TestArcF3ExprTypingOnEveryArm(t *testing.T) {
	...
	// The SET-OPERATION face of the same walk, on every arm.
	//
	// The two shapes whose arms AGREE were #867's fix crossing a UNION; the
	// shapes whose arms DISAGREE were pinned fail-on-agree beside them until
	// #884, because the walk typed a column only when every arm declared it
	// identically and a name left out of that map takes nodeDeclaredType's
	// float fall-through — so `… UNION ALL SELECT NULL` went out as
	// float8/OID 701 where PostgreSQL sends numeric/OID 1700, with the outer
	// `+ 1` lost at int8 magnitude. Both arms of the walk answer from
	// setOpDeclaredOutputSchema now, which skips an UNKNOWN-typed literal arm,
	// folds the rest through setOpWiden and resolves DECIMAL (p,s) through
	// batch.DecimalCommon.
	//
	// The values are the single-table answers this census already asserts
	// above (agg_wide_product, agg_decimal_product), doubled where the query
	// doubles the rows — nothing here is transcribed from a run.
	...
	f3RunOnEveryArm(t, arms, "setop_agg_null_arm",
		`SELECT SUM(v * 3000000) + 1 AS v FROM (SELECT c_i64 AS v FROM typemx `+
			`UNION ALL SELECT NULL) x`, []string{"v=36280278840510000001"})
	f3RunOnEveryArm(t, arms, "setop_agg_int32_and_int64_arms",
		`SELECT SUM(v * 3000000) + 1 AS v FROM (SELECT c_i32 AS v FROM typemx `+
			`UNION ALL SELECT c_i64 FROM typemx) x`,
		[]string{"v=36280387436400000001"})
	f3RunOnEveryArm(t, arms, "setop_agg_decimal_and_null_arms",
		`SELECT SUM(v * 2) + 1 AS v FROM (SELECT c_dec AS v FROM typemx `+
			`UNION ALL SELECT NULL) x`, []string{"v=24750123.7648"})
	// The DECIMAL cross-scale shape, which is the one the filing says was a
	// hard DAG failure as well as a float8 answer: `decpair` carries
	// numeric(9,2) and numeric(18,4), and the union of them is numeric with
	// typmod -1 on PostgreSQL 17.11 (measured through pg_attribute) answering
	// 205.4600. This engine's carrier takes DecimalCommon's (18,4), which
	// prints the same digits.
	f3RunOnEveryArm(t, arms, "setop_agg_decimal_cross_scale",
		`SELECT SUM(v * 2) + 1 AS v FROM (SELECT a AS v FROM `+dbpTable+
			` UNION ALL SELECT b FROM `+dbpTable+`) x`, []string{"v=205.4600"})
	f3RunOnEveryArm(t, arms, "setop_agg_decimal_and_bigint_arms",
		`SELECT SUM(v * 2) + 1 AS v FROM (SELECT a AS v FROM `+dbpTable+
			` UNION ALL SELECT id FROM `+dbpTable+`) x`, []string{"v=196.98"})
