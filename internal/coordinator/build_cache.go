...
// prunedScanColumns intersects stage.Columns with the real table schema before
// building pre-scan SELECT lists: join-chain pruning can include other tables' columns.
// Resolve references through batch.ResolveSchemaIndex and emit the SCHEMA spelling;
// plan names may still be lexer-folded while parquet names retain case (#731).
