...
// snapshotStages copies stage values independently of live *StageInfo pointers.
// Caller must hold qt.mu (either lock); callers read snapshots after unlocking (#514).
// Copy Results into a new slice: RecordResult mutates stages and reassigns that field.
// Drop SeenTasks (nil): it is retry-dedup bookkeeping unused by exported accessors;
// sharing its mutable map would still race even with a copied field.
// Share Dependencies only because it is set before Register and never mutated.
// See docs/internals/query-stage-snapshot-ownership.md for the design.
