...
func TestO1AStarOverAJoinPublishesTheQueryNotThePlan(t *testing.T) {
	...
	c1Run(t, arms, []c1Case{
		...
		},
		{
			...
			pin: map[string]string{
				"single":       "cols=[id:INT64 customer:STRING total:FLOAT64 i.id:INT64 amount:FLOAT64] rows=4 | 1,Alice,150,2,100 | 1,Alice,150,1,50 | 2,Bob,200,4,125 | 2,Bob,200,3,75",
				"spilled512k":  "cols=[id:INT64 customer:STRING total:FLOAT64 i.id:INT64 amount:FLOAT64] rows=4 | 1,Alice,150,2,100 | 1,Alice,150,1,50 | 2,Bob,200,4,125 | 2,Bob,200,3,75",
			...
			why: "DECLINED by design and identical at 0193c4e9: a written LATERAL's subtree " +
				"carries the correlation slot the join drops (ADR-0026 §3c), so the star is " +
				"left to read the stream — which publishes the inner scan's qualifier " +
				"(`i.id`) rather than the lateral's alias, and `ORDER BY l.id` then binds " +
				"nothing and leaves the ties unordered. PostgreSQL publishes `id` and " +
				"orders by it.",
		},
		{
			name: "using-star",
