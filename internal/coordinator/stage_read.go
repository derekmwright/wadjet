...
// fetchResultDataTiered reads a blob through KV, optional producer peer, then S3.
// KV mirrors payloads <= natsKVResultThreshold; its 5-minute TTL and 1 GB cap can miss.
// The peer's LocalStageCache retains the file for the query, written before notification.
// Coordinator-read objects remain durably uploaded under EVERY shuffle-durability
// mode: coordReadStages marks scalar producers and annotateTaskPeerLocations
// keeps uploads eager. The S3 fallthrough may still be waiting for that upload.
// queryID is unused; the peer derives identity from the key's queries/<id>/ prefix.
// tryPeer is true for initial/single-shot reads, false on later fetchStageOutputData
// polls: ask the producer once, then wait for durability without repeated dials.
// See docs/internals/coordinator-tiered-stage-reads.md for the design.
