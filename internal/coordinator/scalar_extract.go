...
// replacePlaceholders replaces :scalar_N tokens with their producers' literals (#659).
// Scan once and consume the MAXIMAL digit run: :scalar_1 must never match
// inside :scalar_10, and map iteration order must not affect values.
// Leave names without literals exactly unchanged; dependency wiring refuses
// placeholders with no producer, and emptying them here would hide that.
// See docs/internals/scalar-placeholder-token-substitution.md for the design.
func replacePlaceholders(s string, literals map[string]string) string {
	const prefix = ":scalar_"
	...
	for i := 0; i < len(s); {
		...
		if j == i+len(prefix) {
			// `:scalar_` with no digits is not a placeholder.
			b.WriteString(s[i:j])
			i = j
			continue
		}
		if lit, ok := literals[s[i:j]]; ok {
			b.WriteString(lit)
		} else {
			b.WriteString(s[i:j])
		}
