...
func TestN1ATwoGroupedLateralsPublishTheirOwnColumns(t *testing.T) {
	...
	f1Run(t, arms, []f1Case{
		...
		{
			// THE SAME WITH `LIMIT 1`, per outer row (#1019, arc LT). PostgreSQL
			// evaluates a lateral per outer row, so its LIMIT bounds each one —
			// two rows here, one per order. The bound travels with the
			// correlation key as a per-key ROW_NUMBER over the grouped body
			// (ADR-0021 §1s); this cell answered `2,Bob,200,Doohickey` alone
			// from 98a90f9d to 51addfb6, and the deferral text that stood here
			// is the arc's proof of the fix.
