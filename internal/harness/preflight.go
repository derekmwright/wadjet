...
// SweepStaleRunArtifacts runs before CheckPreflight to remove abandoned harness
// run directories and dataDir/wadjet/queries intermediates after missed cleanup.
// Caller MUST first checkNoOrphanedWadjet or otherwise prove no concurrent
// process touches those paths. Age alone is not an ownership guarantee.
// pruneOlderThan==0 removes everything in scope; otherwise remove only entries
// older than the cutoff, avoiding freshly created sibling-run artifacts.
// See docs/internals/harness-stale-artifact-sweep.md for the design.
