...
// AuthorizeTableFunction guards destination capabilities, not catalog relations
// (#943, ADR-0034). Nil/disabled auth allows; pure generate_series/unnest allow.
// Otherwise enabled auth without identity refuses; binding errors refuse too.
// ABAC evaluates ActionRead on table_function with name and path/url/host,
// deny-overrides and default deny. Legacy roles require admin, not merely read.
// Server file/network/database access needs explicit privilege; keep the same
// default boundary as the cited pg_read_file/COPY PROGRAM precedent.
// See docs/internals/auth-table-function-capabilities.md for the design.
