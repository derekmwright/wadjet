...
// Decode byte arrays as lengths only when EVERY use provably reads shape, not content.
// Recognized byte-free forms use shapeLenFuncs/ColShapeLen, IS [NOT] NULL, empty-string
// comparisons (ColEmptyStr, emptyStringFilter, ColumnCompare and scan RowPred), or COUNT.
// The historical LENGTH listing is in the design; rune-count functions require bytes.
// Fail CLOSED for the WHOLE query on unhandled AST/plan nodes, more than one scan
// (accumulated names lack qualifiers), joins, DISTINCT, set operations, or wildcard
// scans with no pruned list. Unseen/unclassified uses are not proof of safety.
// A shape-only value reaching a value consumer must panic in batch.BytesColumn.Value,
// never answer from an empty arena.
// See docs/internals/shape-only-column-analysis-boundary.md for the design.
