...
// rewriteDistinctAsGroupBy replaces user Distinct → Project with Project → an
// aggregate-free GROUP BY of its expressions, throughout the WHOLE tree (#163, #466).
// The DAG has no NodeDistinct stage; grouping supplies distributed partial dedup,
// hash partitioning and final merge rather than funneling all rows to the coordinator.
// Leave planner BuildSideDedup alone; it has dedicated physical handling.
// A bare star without a Project goes through rewriteStarDistinct's relation keys.
// Aggregate projections and subquery expressions with no group key fall through:
// MergeInfo.HasDistinct handles the root; elsewhere refuseUnstageableDistinct makes
// PlanDistributed refuse and Coordinator.runDistinctLocal answers locally.
// See docs/internals/distinct-group-by-lowering.md for the design.
