...
// The parser resolves countable SELECT positions; a preceding star makes width unknown.
// Carry deferred ordinals as OrderExpr.Position through resolveOrderBy, then resolve
// immediately after ExpandStarProjections against the expanded projection (#810).
// An ordinal this pass cannot resolve MUST remain unresolved for loud physical refusal;
// never materialize the numeric constant as a sort key, which would silently do no sorting.
// See docs/internals/deferred-star-sort-ordinal-resolution.md for the design.
