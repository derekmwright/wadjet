...
// reduceDecorrelatedScalarAggs reduces aggregate input for joins marked ScalarDecorrelated.
// AFTER predicate pushdown, clone the outer branch producing correlation columns
// with its final filters, then semijoin the aggregate input against its distinct keys.
// Remove only groups whose keys cannot appear in any outer row surviving that branch's
// conjunctive filters/semijoins; such rows die regardless of the scalar's value.
// This holds for EVERY aggregate, including COUNT: absent groups yield NULL through
// the LEFT JOIN either way, and removed groups are never observed.
// See docs/internals/scalar-aggregate-semijoin-reduction.md for the design.
