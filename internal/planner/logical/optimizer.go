...
func sanitizeScanNeeds(n *Node, needs map[string]bool) []string {
	...
	for name := range needs {
		...
		if canon, ok := inSchema[strings.ToLower(name)]; ok {
			keep[canon] = true
		} else if len(inSchema) == 0 || (strings.HasPrefix(name, "__") && !mintedAboveEveryScan(name)) {
			keep[name] = true
		}
	}
...
// mintedAboveEveryScan reports whether name is a hidden slot that only a node
// ABOVE a scan can produce, so a scan naming it is naming nothing.
//
// The "__"-prefixed keep above is deliberate for the families a SCAN FRAGMENT
// itself can emit -- a fused scan-aggregate publishes `__having_N` and
// `__gb_expr_N`, and dropping those would change what the worker's
// all-or-nothing projection guard sees. The correlation families are the
// other case: a decorrelated LATERAL's key slot and a materialized ROW field
// path are minted by a Project or an Aggregate, never by a scan, and they
// arrive on BOTH of a join's scans because the promoted equality names one of
// them. Keeping them there trips that same guard and reverts the scan -- and
// every shuffle fed by it -- to full width.
func mintedAboveEveryScan(name string) bool {
	return plansql.ReservedSlotFamily(name) == string(plansql.SlotCorrKey)
}
