...
func TestParseExpressionCompleteRefusesTrailingText(t *testing.T) {
	for _, tc := range []struct {
		...
		// `#` is the integer XOR operator as of #1179 and `~` the pattern
		// match as of arc PC, so the character that stands for "text this
		// parser does not consume" here is one that is still unlexed.
		{sql: "id > 0 AND n @ 3"},
		{sql: "id = 1 LIMIT 1"},
