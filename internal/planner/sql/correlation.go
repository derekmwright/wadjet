...
// OuterColumnCandidates recursively collects possible enclosing-query reads,
// including correlations nested two levels down, for column pruning (#347).
// Without a catalog, include unqualified names conservatively; callers filter
// against the scan schema with sanitizeScanNeeds. Exclude locally qualified
// references. Extra candidates are safe; dropping a needed outer column is not.
// Unparseable subqueries yield no candidates: compilation must decline them,
// and readOuterValues must fail loudly if an evaluator is nevertheless built.
// See docs/internals/sql-outer-column-pruning-candidates.md for the design.
