...
func FuzzParseSQL(f *testing.F) {
	seeds := []string{
		...
		"SELECT \"id.orig_h\", COUNT(*) AS \"cnt\" FROM \"conn\" WHERE \"proto\" = 'tcp' GROUP BY 1 ORDER BY 2 DESC LIMIT 10",
		"SELECT \"id.orig_h\" AS \"src host\" FROM conn WHERE \"id.resp_p\" = 443 GROUP BY \"id.orig_h\"",
		"SELECT \"a\"\"b\" FROM t WHERE \"x y\" <> \"select\"",
		"SELECT t.\"id.orig_h\" FROM \"my table\" AS t ORDER BY t.\"id.orig_h\"",
		"SELECT \"\" FROM t",
