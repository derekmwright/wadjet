...
// subqueries and DAG scalar producers, which bypass auth.EnforcePlanPolicies.
// Every relation this plan reads asks the context lookup (#945), even with an
// empty resolved policy set: access first, then row-filter/masking obligations.
// Enforce at text-to-plan creation regardless of SELECT/WHERE/CASE/CTE spelling.
