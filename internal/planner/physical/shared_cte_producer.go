...
func assertNoConsumerScopedFilterOnSharedStage(stages []Stage) error {
	...
	for i := range stages {
		...
		// ConsumerScoped records OWNERSHIP, not mere filter/projection presence.
		// Shared producers may keep predicates/projections belonging to the relation
		// itself; attachments belonging to ONE consumer are unsafe. Stage emission and
		// filterCarrierIndex know the distinction; this []Stage walk lacks logical ancestry.
		// Keep the marker and assertion: two filtered scalar producers over one CTE must
		// refuse (TestArcH1TwoFilteredProducersOverOneCTEAreRefusedLoudly, #876).
		// Unconditionally inserting a StageProject at every CTE terminal is not safe (#876).
		// See docs/internals/shared-producer-consumer-ownership.md for the design.
		if !s.ConsumerScoped {
			continue
		}
