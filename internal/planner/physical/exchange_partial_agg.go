...
// markExchangePartialAgg ships name-preserving partials: payload columns are
// group keys or self-mergeable SUM/MIN/MAX under their original names. Require
// a grouped final/merge consumer; all such specs are bare-column SUM/MIN/MAX,
// one function per column across consumers; no COUNT/AVG/DISTINCT/expressions.
// Other consumers must be hash/sort-merge joins whose E-side keys, filters and
// chained/fused keys avoid aggregated columns; all dependents must be grouped
// aggregates re-aggregating covered columns with the same functions.
// Payload must be exact, without ComputedCols/ExtraReadCols; partition keys
// stay group keys. Run after all rewiring; WADJET_EXCHANGE_PARTIAL_AGG=0 disables.
// See docs/internals/exchange-partial-aggregation.md for the design.
