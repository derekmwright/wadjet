...
// markDimensionCascade wires D's key bloom into primary build scan B0, then
// emits the surviving build keys to fact probe F. The chained join's key must
// originate in B0 (BuildColOrigins/column membership). Hop A forces B0 through
// dispatchScanFilterStage; AtOutput must observe POST-consume rows.
