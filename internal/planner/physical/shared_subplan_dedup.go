...
// dedupeSharedSubplans rewires duplicate subtree consumers to a surviving root
// and removes orphans. Fingerprints exclude Columns/OutputColumns and scan aliases;
// kept scans must read a superset so rewired consumers still resolve their names.
// Exact matches allow any consumer. Semi→inner matches require same children/keys,
// no JoinFilter, and EVERY consumer an aggregate with GroupByCols containing the
// probe keys and only duplication-invariant avg/min/max: no build-uniqueness oracle
// exists, and duplication must be constant per group. Run after final distribution
// but before fuseStageChains; shared outputs and per-query cleanup already suffice.
// See docs/internals/shared-subplan-deduplication.md for the design.
