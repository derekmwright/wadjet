...
// SELECT-list scalar subqueries use :scalar_N producer-stage deferral (#659),
// matching predicate accumulation order; never execute and splice at plan time.
// Keep ProjectExprSpec.Name's original item text for logical OutputRenames while
// Expr carries the placeholder. Type exactly as the single path does, not from
// the producer; never claim TypeKnown without a known type (zero is BOOLEAN).
// Decline surviving unwalked subqueries, correlation, values without lossless
// literal spelling, and dependencies awaiting a producer that reads the consumer.
// Refusals retain coordinator-local execution (ADR-0021,
// ErrScalarSubqueryProjectionDistributed / ErrCorrelatedSubqueryDistributed).
// See docs/internals/scalar-select-list-deferral.md for the design.
