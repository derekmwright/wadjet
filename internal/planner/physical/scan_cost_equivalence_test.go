...
func TestTheLogicalScanCostMatchesTheStageCost(t *testing.T) {
	...
	type pair struct{ logical, stages QueryCost }
	pinned := map[int]pair{
		17: {QueryCost{TotalBytes: 13002342400, TotalRows: 124000000, TotalFiles: 1240},
			QueryCost{TotalBytes: 6501171200, TotalRows: 62000000, TotalFiles: 620}},
		18: {QueryCost{TotalBytes: 14313062400, TotalRows: 136500000, TotalFiles: 1365},
			QueryCost{TotalBytes: 8021606400, TotalRows: 76500000, TotalFiles: 765}},
		21: {QueryCost{TotalBytes: 20468203520, TotalRows: 195200000, TotalFiles: 1952},
			QueryCost{TotalBytes: 14176747520, TotalRows: 135200000, TotalFiles: 1352}},
		22: {QueryCost{TotalBytes: 1730150400, TotalRows: 16500000, TotalFiles: 165},
			QueryCost{TotalBytes: 1887436800, TotalRows: 18000000, TotalFiles: 180}},
	}
	...
	for qNum := 1; qNum <= 22; qNum++ {
		...
		t.Run(fmt.Sprintf("Q%02d", qNum), func(t *testing.T) {
			...
			if want, ok := pinned[qNum]; ok {
				got := pair{QueryCost{fromLogical.TotalBytes, fromLogical.TotalRows, fromLogical.TotalFiles, false, false},
					QueryCost{fromStages.TotalBytes, fromStages.TotalRows, fromStages.TotalFiles, false, false}}
				if got != want {
