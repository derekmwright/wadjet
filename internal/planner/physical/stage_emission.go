...
func (p *StagePlanner) walkStages(node *logical.Node, stages *[]Stage, parentID *string) {
	...
	case logical.NodeJoin:
		...
		stageKeyTypes := resolveJoinKeyTypes(node, leftKeys, rightKeys, p.cteKeyColTypes)
...
func absorbSecurityBarrier(node, scan *logical.Node, stages *[]Stage) {
	...
	for _, pr := range node.Projections {
		...
		if isExpr {
			...
			materialized := declTypeParts(inferProjectionDeclType(pr.ASTExpr, parquet.TypeString,
