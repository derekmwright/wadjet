...
// cpuTokens bounds EXTRA compute goroutines; each fragment keeps a free baseline
// (docs/design/morsel-execution.md §4.2). TryAcquire never blocks.
// Only consumers already holding admitted work may enqueue FIFO (§4.2.1):
// other bounded decode/consumer holds release capacity, never their own progress.
// Decode can take its reserved floor ahead of FIFO; holdback is bounded by
// registered demand and reserve <= capacity-1. With fedWaiters==0, decode may
// outrank FIFO without the floor cap; a fed waiter restores priority beyond it.
// These bounds and the runnable token-free baseline preserve liveness.
// WADJET_DECODE_ADMISSION=0 restores no reserve/flip and decode behind FIFO.
// See docs/internals/worker-cpu-token-admission.md for the design.
