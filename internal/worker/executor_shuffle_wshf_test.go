...
func TestExecuteShuffle_WSHFInput(t *testing.T) {
	...
	s1 := distributed.Task{
		...
		ColumnTypes:   declaredFromRows(rows),
...
// TestExecuteShuffle_WSHFInputColumnProjection is the worker-side regression
// test for the WSHF-input shuffle projection (fix 2 of the scan-output
// column leak): a chained shuffle whose task declares Columns must drop
// undeclared columns from the decoded upstream payload before
// re-partitioning, while sibling-table names in the declaration (planner
// over-approximation convention) are ignored and shuffle keys survive.
// Before the fix, task.Columns was parquet-only and the full upstream
// width re-materialized.
func TestExecuteShuffle_WSHFInputColumnProjection(t *testing.T) {
	ctx := context.Background()
	const bucket = "test-shuffle-wshf-prune"
	const numRows = 200
	...
	for i := 0; i < numRows; i++ {
		rows[i] = map[string]any{
			"k":     int64(i),
			"v":     fmt.Sprintf("value-%d", i),
			"extra": fmt.Sprintf("padding-that-should-not-ride-the-shuffle-%d", i),
		}
	}
	writeParquetFile(t, store, bucket, "tables/src/part-0.parquet", rows)
	...
	// Stage 1: full-width shuffle parquet → .wshf (no Columns declared).
	s1 := distributed.Task{
		ID:            "s1",
		QueryID:       "q-prune",
		StageID:       "shuffle-1",
		...
		DataBucket:    bucket,
		ResultBucket:  bucket,
		ResultPrefix:  "shuffle/q-prune/s1/",
	}
	r1 := executor.Execute(ctx, s1, "w1")
	if !r1.Success {
		t.Fatalf("stage 1 failed: %s", r1.Error)
	...
	// Stage 2: chained shuffle over the .wshf with a declared projection.
	// "c_sibling" doesn't exist in the payload — the planner's shuffle
	// column unions include the other join side's names by convention.
	s2 := distributed.Task{
		ID:            "s2",
		QueryID:       "q-prune",
		StageID:       "shuffle-2",
		Type:          distributed.TaskTypeShuffle,
		Files:         r1.ResultFiles,
		Columns:       []string{"v", "c_sibling"},
		ShuffleKeys:   []string{"k"},
		NumPartitions: 2,
		DataBucket:    bucket,
		ResultBucket:  bucket,
		ResultPrefix:  "shuffle/q-prune/s2/",
	}
	r2 := executor.Execute(ctx, s2, "w1")
	if !r2.Success {
		t.Fatalf("stage 2 failed: %s", r2.Error)
	...
	if r2.NumRows != numRows {
		t.Errorf("stage 2 rows: got %d, want %d", r2.NumRows, numRows)
	}

	// Every output batch must carry exactly {k, v}: "extra" dropped,
	// key "k" kept despite not being declared.
	var totalBack int64
	for _, key := range r2.ResultFiles {
		rc, _, err := store.Get(ctx, bucket, key)
		if err != nil {
		...
		for {
			...
			got := make(map[string]bool, len(b.Schema))
			for _, c := range b.Schema {
				got[c.Name] = true
			}
			if len(b.Schema) != 2 || !got["k"] || !got["v"] {
				t.Fatalf("output schema in %q = %v, want exactly [k v]", key, got)
			}
			totalBack += int64(b.ActiveLen())
		}
	}
