...
type unpartitionedStageSink struct {
	...
	// Chunk size belongs to the SINK's row/byte thresholds, not consume granularity.
	// For the shared accumulator, coalesce is 0 undecided, 1 flat-schema coalescing,
	// -1 legacy nested chunk-per-consume (#397).
	// Swap full rowBuf/spareBuf under mu; encode/write outside the lock.
	// flushing permits one writer; flushCond backpressures a second full buffer,
	// keeping this path at two accumulators (docs/design/morsel-execution.md §4.1.1).
	// Legacy writes retain mu and never set flushing; modes are exclusive.
	// See docs/internals/worker-stage-sink-coalescing-state.md for the design.
	coalesce   int8
