...
func TestReaderAtClosePerIteration(t *testing.T) {
	...
	for i := 0; i < numFiles; i++ {
		w := parquet.NewWriter(schema)
		for j := 0; j < 10; j++ {
			w.WriteRow(map[string]any{"id": int32(i*10 + j)})
		}
		data, err := w.Finish()
		if err != nil {
			t.Fatalf("writing parquet file %d: %v", i, err)
		}
		paths[i] = "data/file" + string(rune('0'+i)) + ".parquet"
		if err := mem.Put(ctx, bucket, paths[i], data); err != nil {
	...
	for _, filePath := range paths {
		var raCloser objstore.ReaderAtCloser
