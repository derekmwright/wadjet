...
// partitionedShuffleSink hashes to N partition streams; threshold flushes
// bound accumulators/spares at N × 2 × flush threshold, independent of total input.
// Upload under stable partition prefixes read by partitionShardSource.
// Consume locks per PARTITION; hashing/scatter use private pooled scratch.
// A partition slice at/above the byte threshold may encode directly from the
// source outside pw.mu, guarded by pw.flushing; lock only handoff/counters.
