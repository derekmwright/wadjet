...
// claim prefers the fragment baseline, a free token, then FIFO; caller holds
// an admitted morsel (cpuTokens.enqueueWaiter supplies the liveness argument).
// fed reports queued morsels behind the held one; dry waiters permit decode
// priority, fed waiters restore FIFO beyond decode's reserve (cpu_tokens.go).
// A grant may be donated to this fragment's token-stalled scanner at most ONCE
// per claim (§2.3), then re-enqueued; the second grant sticks.
// This covers starvation with no held-token yield (§2.2). Donation goes directly
// grant→tryDonateToken→decode→release, never back through the pool.
// See docs/internals/worker-width-claim-token-donation.md for the design.
