...
// Unpartitioned producers append into checked-out local slabs without a shared
// row-copy lock; full slabs become chunks without a second accumulator copy.
// Hold the sink lock only for stream ownership handoff and counters.
// Chunk thresholds remain the SINK's flushRows/flushBytesT decision, preserving
// serial chunk boundaries (docs/design/morsel-execution.md §4.1.1).
// stageSlabBudgetFactor bounds aggregate buffered bytes under parallel writers;
// do not equate consume/morsel size with chunk size.
// See docs/internals/worker-producer-local-stage-accumulation.md for the design.
