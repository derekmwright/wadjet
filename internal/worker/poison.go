...
// Persist a NATS KV breadcrumb immediately before task execution and clear it
// only after graceful success/published failure (#318).
// Delivery 1 or redelivery without a record executes normally. Redelivery with
// an undegraded record uses reduced memory to engage spill (ADR-0006, #325).
// If the recorded degraded attempt also died, quarantine: fail with task/memory
// diagnosis and Term the message so redelivery stops.
// AckWait can misclassify a healthy slow task; degraded duplicate execution is
// safe because TaskID outputs are idempotent and scheduling/spill preserve rows.
// WADJET_POISON_DEFENSE=0 disables this mechanism.
// See docs/internals/worker-poison-task-delivery-defense.md for the design.
