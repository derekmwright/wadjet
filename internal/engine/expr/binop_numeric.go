...
// BinOpNumeric resolves arithmetic mode once against the first batch.
// Plain Int32/Int64 operands and integer literals use integer arithmetic;
// other domains follow their resolved decimal or float path.
// Int64 overflow raises 22003, never a wrapped value (#637).
// Integer division truncates toward zero with either int-arith toggle setting:
// the disabled float delegate uses divTrunc (#369, ADR-0012).
// The toggle controls +, -, *, % typing, never division semantics.
// See docs/internals/integer-arithmetic-mode-and-toggle.md for the design.
