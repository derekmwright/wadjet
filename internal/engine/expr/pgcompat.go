...
// These scalar shims serve FROM-less SELECTs and real-table calls beyond
// pgwire's synthetic statement intercepts (#341).
// At this layer privileges answer granted, visibility visible and comments
// none; shims cannot inspect catalog/storage state or enforce authorization.
// Leave pg_get_functiondef, pg_get_partkeydef, pg_get_serial_sequence,
// pg_relation_size, pg_total_relation_size, pg_tablespace_location,
// to_regclass and pg_sleep unshimmed so unresolved calls error.
// A constant NULL to_regclass would falsely report every table missing.
// See docs/internals/pgcompat-scalar-shim-boundary.md for the design.
func init() {
