...
// Temporal parser memos must be bounded: their strings come from DATA,
// not the fixed set of query literals (#619).
// Bare column/literal comparisons specialize to CmpTemporalLit and use
// uncached compile-time parsing; unspecialized row comparisons reach here.
// Reset the entire generation at temporalMemoCap to collapse scan repetition
// without LRU bookkeeping; concurrent in-flight stores may overshoot.
// See docs/internals/bounded-temporal-string-memo.md for the design.
