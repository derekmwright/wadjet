...
// quotedNumberOrder coerces an unknown quoted literal to the NUMBER's own type
// and compares without widening (#646); TYPE decides, never the widened Go box.
// ColRef boxes float4 as float64 and int4 as int64; restore REAL width bit-exactly
// and enforce int4 range checks instead of comparing at box width.
// numberKindType uses the declaration when known, otherwise the box for literals
// and computed numerics after KIND separates DECIMAL/STRING (ADR-0012 item 8).
// A literal refused by the type must RAISE, never fall through to a boolean answer.
// See docs/internals/quoted-number-declared-width-order.md for the design.
