...
// The row path must match the REAL-width IN kernel (#633, #549).
// Narrow only a REAL-typed probe with more than one syntactic member,
// counting NULLs, and members accepted by realListMember.
// Numeric, quoted and mixed literals use the same float input grammar (#646);
// explicit REAL casts retain narrowing. Wider casts/nonconstants decline.
// Single numeric literals use scalar double-width comparison.
// Select probe type through realTypedOperand, not its widened Go box.
// Refusals name quoted TEXT verbatim but expand numeric literal DIGITS.
// See docs/internals/real-in-list-row-width-contract.md for the design.
