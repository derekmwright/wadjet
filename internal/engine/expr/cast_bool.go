...
// CAST AS BOOLEAN selects its rule from the operand declaration, never
// from a column's Go box (ADR-0012 items 1, 5 and 8; #592, #361).
// BOOL is unchanged; Int32/Int64 zero is false and every other value true.
// STRING uses parseBoolText and raises 22P02 for invalid boolean text.
// NULL stays NULL for every source type; other declared types raise 42846.
// DECIMAL text must not take STRING's parser, nor DATE/network encodings
// the integer arm. Unresolved declarations use castBoolByBox's fallback.
// See docs/internals/boolean-cast-source-domain.md for the design.
