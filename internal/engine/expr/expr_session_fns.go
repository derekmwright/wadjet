...
// fnCurrentSchemas returns the search path as a text array; PostgreSQL
// uses name[]. include_implicit prepends pg_catalog, and a NULL argument
// returns NULL. See ADR-0044 for the catalog declaration difference.
...
// formatInstant is the shared instant renderer: batch.FormatTimestamp over
// UTC epoch milliseconds, with a fractional second only when present (#544).
// Callers may truncate in the parsed location first; rendering changes no instant.
// to_iso8601 keeps its named format, and at_timezone keeps the offset needed
// to read its wall clock correctly; neither uses this renderer.
// Finer precision is lost at the millisecond boundary, as on the wire
// (docs/data-types.md, "One rendering").
// See docs/internals/shared-instant-rendering-contract.md for the design.
