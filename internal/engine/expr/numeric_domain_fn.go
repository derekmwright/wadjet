...
// ABS preserves integer/REAL domain; MOD preserves the common integer
// domain, with matching declaration and kernel (#768).
// Do not extend this rule to CEIL/FLOOR/ROUND/TRUNC/SIGN or transcendental
// functions: their non-decimal results stay double precision.
// The exact DECIMAL same-domain family is separate (#668).
// FLOAT MOD keeps its existing float superset answer; integer MOD uses
// integer remainder, never math.Mod's signed zero.
// See docs/internals/abs-mod-own-numeric-domain.md for the design.
