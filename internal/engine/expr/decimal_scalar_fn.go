...
// abs/ceil/floor/round/trunc/sign/mod over DECIMAL compute in their OWN exact domain
// (ADR-0024 items 2 and 3, #668), never round-trip rendered text through float64.
// The typed replacement retains FuncCall as fallback; resolve against batch types
// and delegate every non-DECIMAL argument with bit-identical semantics.
// sqrt/exp/ln/log/power deliberately remain float64: PostgreSQL numeric support
// would require an exact fixed-point implementation (ADR-0012 item 9 divergence).
// See docs/internals/exact-decimal-scalar-math.md for the design.
