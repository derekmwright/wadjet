...
// chargeMemory reserves the retained membership set's estimated footprint
// once under resolveSlow's held resolveMu (ADR-0006, #528).
// A nil Budget is a no-op. The non-spilling set counts against later task
// allocations; a failed reservation raises a query error.
// Charging happens AFTER construction and cannot prevent a build-time OOM.
// Bounding peak residency requires incremental reservations and partial-failure
// handling during accumulation, outside this function's present contract.
// See docs/internals/in-subquery-post-build-budget-charge.md for the design.
