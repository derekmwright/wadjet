...
// Page-cache pressure uses workingset refaults, not cold first-touch faults;
// see docs/design/scan-decode-pipelining.md §9.
// Prefer process cgroup-v2 memory.stat, else host /proc/vmstat; shared-host
// co-tenant pressure may reduce width. No readable source permanently disables it.
// Sample at most once/second. Two consecutive above-threshold samples activate;
// one quiet sample deactivates. Apply any enabled designed-streaming discount
// before thresholding; Go heap pressure alone cannot observe cache displacement.
// See docs/internals/memory-refault-pressure-sensor.md for the design.
