...
// TestWaitForHeapHeadroom_NoPressure confirms the helper returns near-
// immediately (zero wait) when HeapBackpressureActive is false.
func TestWaitForHeapHeadroom_NoPressure(t *testing.T) {
	resetHeapBackpressureCache(t)
	t.Cleanup(func() { resetHeapBackpressureCache(t) })

	start := time.Now()
	waited, err := WaitForHeapHeadroom(context.Background())
	elapsed := time.Since(start)
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
	if waited > 5*time.Millisecond {
		t.Errorf("expected no wait when no pressure; got %v", waited)
	}
	if elapsed > 10*time.Millisecond {
		t.Errorf("WaitForHeapHeadroom too slow with no pressure: %v", elapsed)
	}
}

// TestWaitForHeapHeadroom_WaitsThenAdmits spoofs pressure on, then clears
// it after a short delay from a separate goroutine. WaitForHeapHeadroom
// should block until pressure clears, then return.
func TestWaitForHeapHeadroom_WaitsThenAdmits(t *testing.T) {
	resetHeapBackpressureCache(t)
	heapBackpressureMu.Lock()
	heapBackpressureLastValue = true
	heapBackpressureLastCheck = time.Now().Add(time.Hour) // freeze cache fresh
	heapBackpressureMu.Unlock()
	t.Cleanup(func() { resetHeapBackpressureCache(t) })

	// Clear pressure after a short delay.
	const clearAfter = 400 * time.Millisecond
	go func() {
	...
	// Expect wait >= clearAfter (we polled at least once after pressure cleared)
	// and <= clearAfter + 1 poll interval + scheduler jitter.
	if waited < clearAfter {
		t.Errorf("expected wait >= %v; got %v", clearAfter, waited)
	}
	if waited > clearAfter+AdmissionPollInterval+200*time.Millisecond {
		t.Errorf("waited too long after pressure cleared: %v (clearAfter=%v poll=%v)",
			waited, clearAfter, AdmissionPollInterval)
	}
	if elapsed < clearAfter {
		t.Errorf("elapsed wall %v < expected clearAfter %v", elapsed, clearAfter)
	}
}

// TestWaitForHeapHeadroom_RespectsContext confirms cancellation returns
// promptly even while heap pressure remains active.
func TestWaitForHeapHeadroom_RespectsContext(t *testing.T) {
	resetHeapBackpressureCache(t)
	heapBackpressureMu.Lock()
	heapBackpressureLastValue = true
	heapBackpressureLastCheck = time.Now().Add(time.Hour)
