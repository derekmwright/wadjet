...
// projSourceName chooses the same source spelling as the schema pass: DirectCopy,
// then SourceCol, then Name for a noncomputed projection; computed-only has none.
// resolvePlainColumn's contract is idx>=0 for bulk copy, idx=-1/ok=true for per-row
// ROW-field evaluation, and ok=false for an unresolved name that must error (#147).
// Resolve ROW parents before qualified/bare columnIndexFallback so a field cannot
// bind to an unrelated bare column. Value lookup and schema lookup must agree (#314).
// See docs/internals/projection-source-and-field-resolution.md for the design.
