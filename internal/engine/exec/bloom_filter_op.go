...
// keyTypesAgree must check STORAGE and ENCODING separately; mismatch disengages filtering.
// Integer fast paths require isIntKeyColumn before indexing Int32Data/Int64Data,
// even for wire blooms without builder metadata. TIMESTAMP/IPv4/MAC/DURATION
// pass that storage check, but storage compatibility alone is not key compatibility.
// With builder metadata, the resolved column must encode like the inserted column:
// bloomIntKey covers only the five types outside appendColumnValue's encoding.
// An INT64-built bloom must reject TIMESTAMP despite shared Int64Data storage;
// never widen the encoding predicate to the storage predicate.
// See docs/internals/bloom-key-storage-and-encoding-guards.md for the design.
