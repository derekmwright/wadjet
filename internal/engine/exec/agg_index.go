...
// convertsToTwoLevel tests only at batch end, never per row: live groups must
// reach twoLevelConvertAt and live+incoming must cross the flat table's 70% load.
// Convert when replacing an imminent rehash; otherwise conversion adds a rehash
// without replacing the flat doubling. A saturated table with no new groups stays flat.
// Consume estimates incoming from the just-finished batch's new-group count;
// merge supplies the exact incoming group count. See two_level_hash.go.
// The toggle must be on; the eager test/threshold override bypasses lookahead.
// See docs/internals/two-level-conversion-timing.md for the design.
