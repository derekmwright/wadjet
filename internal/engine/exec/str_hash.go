...
// strHashTable maps byte-slice keys to int32 in an open-addressing GROUP BY table.
// Keys are copied once into on-demand, fixed-size chunks; stored bytes never move
// or mutate. Interior references keep chunks alive, and callers may alias stored
// keys through GetOrInsertRef (including aggregate serializedKeys via arenaString).
// strEntry.ref packs chunk index in high bits and byte offset in low bits.
// Keys never span chunks: insufficient tail space starts a new chunk; an oversized
// key gets its own exact-size chunk. Empty slots have keyLen < 0.
// See docs/internals/string-hash-chunked-key-arena.md for the design.
