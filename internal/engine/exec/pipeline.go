...
// SinkSurvivesCloning fences morsel-parallel clones whose state cannot merge safely.
// Every CloneSink caller, including Pipeline.runParallel and worker breaker consume,
// must consult it; TestCloneSinkCallersConsultTheCloneFence enumerates those sites.
// Do not merge pre-folded DISTINCT accumulators: unioning sets cannot undo duplicate
// SUM/mean/variance contributions already accumulated by different clones (#703, #291).
// Consult the predicate below for the enforced DISTINCT fence; non-distinct state
// can merge. The stage DAG uses one-level RawInputAggregate for distinct aggregates;
// raw-row spill re-aggregation is unaffected.
// See docs/internals/sink-clone-merge-fence.md for the design.
