...
// hashOnceToggle threads the sink's group-key hash through routing and slot lookup.
// Owner = high half of hash*parts; slot = hash & (cap-1). Keep high partition
// and low slot windows disjoint and low bits uniform within each owner interval.
// The design assumes <=4096 workers (<=12 high bits) and tables below 40 slot bits.
// Use fibHash for int64 (high-bit fold then phi multiply, retaining dense-key locality
// and spreading 2^s multiples; #306), packedHash(lo, hi) for packed keys, strHash for strings.
// strHash's low 32 bits also provide strEntry.hashTag.
// Preserve the sink tables' slot sequence; do not substitute router mix64/fnv1a.
// See docs/internals/aggregate-shared-routing-hash.md for the design.
