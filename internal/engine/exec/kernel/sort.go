...
// CompareDecimalAt orders exact NUMERIC values at every scale (#394).
// Equal scales compare Int128 directly. Otherwise multiply the smaller-scale
// operand by 10^delta, never scale down; on Int128 overflow compare as big.Int.
// Never approximate through float64: sort-merge join uses comparator zero as
// key equality, so rounding can create spurious matches.
// Share CompareDecimalValues with column-column filters and sorted-run merges.
// See docs/internals/kernel-exact-decimal-ordering.md for the design.
