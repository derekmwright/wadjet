...
// StatsDomainValue converts literals to the parquet statistics/dictionary
// domain selected by declared type and scale (#442, #438).
// DECIMAL needs scaled carriers/residual rules; IPv6/UUID need raw byte keys,
// not display text. Match the filter's conversion rather than rendering bounds.
// ok=false means WITHHOLD the entire predicate from pruning.
// Name every TypeID explicitly; no raw pass-through default for new types,
// which could silently compare two same-kind boxes with different meanings.
// See docs/internals/kernel-statistics-literal-domain.md for the design.
