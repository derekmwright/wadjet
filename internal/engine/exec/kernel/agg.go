...
// sumSliceExactInt64 computes the true batch total in signed Int128, then
// checks whether it fits int64: hi must equal int64(lo)'s sign extension.
// A temporary per-row overflow that later cancels must not fail the batch;
// ResolveBatchSum's cross-batch fold retains its conservative overflow rule.
// This serves computed integer and TIMESTAMP/DURATION sums; bare integer
// column SUM uses Int128 or widened int32 accumulation (#784).
// See docs/internals/kernel-exact-int64-batch-sum.md for the design.
