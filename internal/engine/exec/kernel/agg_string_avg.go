...
// Byte-backed MIN/MAX compares zero-copy views and copies only a new winner
// out of the batch. Retain Accumulator.StrType so raw IPv6/UUID bytes box as
// []byte, while text stays text (#417); type dispatch must match sort order.
// BOOL has explicit MIN/MAX updaters too.
// Float AVG kernels avoid int64 running-sum wrap and set IsFloat so finalize,
// merge and partial spill use the float sum. Only isAvgFloatAccumType selects
// this path; exact integer SUM/AVG rules remain with their own resolvers.
// See docs/internals/kernel-byte-minmax-and-float-avg.md for the design.
