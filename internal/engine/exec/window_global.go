...
// Global windows stream sorted runs twice: collect partition scalars, then reopen
// and emit incrementally, matching computePartitionColumnar's values.
// With ORDER BY and no explicit frame, RANGE UNBOUNDED PRECEDING TO CURRENT ROW
// ends at the peer group's end; backfill peer-close columns identically for every
// peer, never expose per-row running values (#350).
// Lead, cume_dist and peer-close columns hold rows back; memory is bounded by max
// lead offset plus the largest ORDER-BY peer group and is charged to the tracker.
// All-equal keys can require the full partition. Explicit frames route through
// groupNeedsMaterializedFrame to the partition-at-a-time walker.
// See docs/internals/global-window-two-pass-streaming.md for the design.
