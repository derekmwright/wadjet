...
// HashAggregate spills sorted group keys plus accumulators; Finalize k-way merges
// runs and combines equal keys. See the design for the little-endian WAGS format.
// Every run must use the drain cursor's appendSerializedKey/appendKeyValue encoding,
// not appendColumnValue's different in-memory hash format. Sort/merge by those keys,
// which must distinguish unequal groups and serialize comparator-equal values alike.
// Group VALUES have separate encodings: merge keys irreversibly fold NaNs/-0 and
// re-key CIDR. Flat values use tagged scalars; ARRAY/ROW/MAP/VECTOR use the lossless
// partialTagContainer tree in aggregate_container_key.go (#566, #576).
// See docs/internals/aggregate-partial-spill-format.md for the design.
