...
// Execute applies OFFSET then Max using each atomic Add's returned claim
// [end-n,end), never a Load/decide/Add sequence. Concurrent batches own disjoint
// positions and contribute exactly their overlaps with [Offset,∞) then [0,Max).
// Both skip and limit counters need this rule to prevent over/under-skipping
// and over-delivery (#567, #765, #845).
// Which unordered rows survive may depend on scheduling (ADR-0013 classes 1
// and 3); the number of rows must be exact.
// See docs/internals/parallel-limit-offset-claims.md for the design.
