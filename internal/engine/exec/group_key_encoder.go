...
// All merge-key producers share one encoder per declared column TYPE
// (ADR-0023 item 5, ADR-0026): equal values must produce identical merge bytes.
// Int/packed SoA drains, compact/generic boxed drains, migrateToGenericMap and
// decodeSerializedKey must agree despite their different boxes (#788).
// Key on comparator storage, never display or Go box type (ADR-0023 item 1),
// including DATE/IPv4/MAC and INT32/PORT/PROTOCOL.
// Emitted values keep their boxed round trip (ADR-0023 items 2-3);
// never decode a value from a merge key.
// See docs/internals/declared-type-group-merge-key.md for the design.
