...
// Container payloads preserve ARRAY/ROW/MAP/VECTOR recursive shape (#397).
// Worker and coordinator WSHF decoders share this spill-derived framing.
// BYTES leaves deliberately use WSHF null-skipping totalLen/data/n END offsets;
// raw offset pairs may descend at NULL slots and must not be copied blindly.
// The outer column bitmap is outside this payload; child bitmaps are inside.
// NULL and empty containers both have equal adjacent offsets: the column
// null bit must keep them distinct.
// See docs/internals/batch-nested-column-payload.md for the design.
