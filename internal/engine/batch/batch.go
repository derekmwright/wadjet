...
// MintStamp identifies a producer and each issue of its storage without
// retaining batch pointers; never replace it with a strong-reference registry.
// Owner is process-unique from NewMintOwner; zero is foreign/unstamped and
// must not be adopted, which would create a second recycler.
// Seq increments on every reissue; releases carry their captured Seq and stale
// releases must be ignored, never returning live storage to the free list.
// The producer writes under exclusive ownership; publication channels and the
// pool mutex order reads. See docs/design/scan-output-backing-reuse.md.
// See docs/internals/batch-mint-stamp-storage-identity.md for the design.
