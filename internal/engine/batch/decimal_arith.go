...
// Fixed-point arithmetic follows docs/adr/0024 and ADR-0024 items 3 and 4:
// compute exactly, rounding HALF AWAY FROM ZERO once at outScale.
// A non-DecimalOK status requires its SQLSTATE; never use the returned value.
// DecimalOK proves Int128 CARRIER fit only, not declared precision.
// Use DecimalAddAt/SubAt/MulAt/DivAt/ModAt for DecimalFitsPrecision too;
// bare-op callers must supply any declared precision bound themselves.
// Use the Int128 fast path and exact big.Int fallback only when an intermediate
// overflows; both compute the same rule without float narrowing or saturation.
// See docs/internals/batch-exact-decimal-arithmetic.md for the design.
