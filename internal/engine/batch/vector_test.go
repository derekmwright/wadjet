...
func TestFormatDateAndParseDateString(t *testing.T) {
	...
	// #451: parseDateString computed the day count via t.Sub(epochDate), a
	// time.Duration that saturates at ±math.MaxInt64 ns (~292 years) rather
	// than reporting an overflow, so any 4-digit-year date outside roughly
	// 1677-09-22..2262-04-11 silently answered the window's edge instead.
	// Values below are PostgreSQL-verified (`date '<lit>' - date
	// '1970-01-01'`).
	t.Run("parseDateStringDoesNotClampFarDates", func(t *testing.T) {
		tests := []struct {
			lit  string
			days int32
		}{
