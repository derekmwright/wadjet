...
func runSelDecodeDifferential(t *testing.T, fr *pqt.FileReader, schema []pqt.Column) {
	...
	for rg := 0; rg < fr.NumRowGroups(); rg++ {
		...
		for name, sel := range selShapes(n, r) {
			got, err := readRowGroupNative(fr, rg, schema, nil, nil, sel)
			if err != nil {
...
func BenchmarkSelDecode(b *testing.B) {
	...
	b.Run("full", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			if _, err := readRowGroupNative(fr, 0, schema.Columns, nil, nil, nil); err != nil {
				b.Fatal(err)
			}
		}
	})
	b.Run("sel1pct", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			if _, err := readRowGroupNative(fr, 0, schema.Columns, nil, nil, sel); err != nil {
				b.Fatal(err)
			}
		}
	})
	...
	b.Run("clustered1pct", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			if _, err := readRowGroupNative(fr, 0, schema.Columns, nil, nil, clustered); err != nil {
				b.Fatal(err)
			}
		}
	})
	...
	b.Run("sparse5rows", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			if _, err := readRowGroupNative(fr, 0, schema.Columns, nil, nil, sparse); err != nil {
				b.Fatal(err)
			}
		}
	})
