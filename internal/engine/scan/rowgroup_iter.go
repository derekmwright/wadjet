...
// RowGroupIter decodes one row group on demand, bounding source storage to
// one decoded group plus downstream in-flight batches; do not eager-decode a file.
// Open, defer Close, call Next until nil/error, and Release batches after use.
// Callers must check HasUnsupportedColumnarTypes and use ReadFileBatchesShard
// for ARRAY/MAP and unsupported nested ROW shapes; the row fallback retains
// its existing whole-file/sharding behavior.
// See docs/internals/scan-row-group-iterator-lifecycle.md for the design.
