...
// HasUnsupportedColumnarTypes refuses ARRAY/MAP and ROW fields that are
// ROW/ARRAY/MAP; native ROW lookup addresses direct primitive leaves only.
// Route unsupported shapes through ReadFileBatchesShard's row assembler,
// never treat deeper group paths as missing/all-NULL (#448, #425, #409).
// DECIMAL is supported. Keep one recursive assembly rule (ADR-0018 §3).
// Planner eager/top-N gates may reject any ROW earlier; worker cached streams
// must still apply this predicate before opening the native iterator.
// See docs/internals/scan-native-container-boundary.md for the design.
