...
// DecodedChunkCache owns immutable clones; admit clones and hits copy into
// caller batches, never sharing mutable storage. GC protects copies after eviction.
// Keys include object identity, RG, leaf and catalog declaration, including
// DECIMAL scale/precision. See docs/design/decoded-rowgroup-cache.md.
// Charge cache bytes once via Size/reservoir and AccountedOperator relief
// (ADR-0006); consumer copies remain separately charged scan output.
// Segmented LRU uses ghosts and frequency-gated admission; hits promote to
// protected, and aging prevents permanent incumbents. Cold floods must not
// indiscriminately replace the hot set.
// See docs/internals/scan-decoded-chunk-cache-ownership.md for the design.
