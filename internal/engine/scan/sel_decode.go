...
// Sel-aware decode materializes only selected flat STRING/BYTES rows (#299).
// Write offsets for EVERY row, monotonic with zero-length dead slots; only
// selected rows gather/copy bytes, directly from dictionaries when possible.
// Fixed-width columns keep full memmove. Never offer selected-only columns
// to the decoded cache: its key has no selection, so hits would corrupt full reads.
// Bounds-check every dereferenced dictionary index; malformed files must error,
// never panic or mis-slice, matching the full decoder's failure contract.
// See docs/internals/scan-selected-byte-materialization.md for the design.
