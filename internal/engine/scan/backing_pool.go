...
// BackingPool reuses a backing only when RELEASED and UNCLAIMED: retire must
// follow every view's whole op chain and sink consume (ADR-0015), while Detach/
// Claim through derived shells, Base or children vetoes reuse (ADR-0016).
// One claim surrenders the WHOLE backing permanently; neither condition alone
// protects concurrent readers and retained values.
// Accept only this pool's current batch.MintStamp generation; foreign/stale
// releases must not create another owner. Keep no outstanding-pointer registry.
// Create a pool only with a release edge (worker.batchRecyclerOf); dropped
// batches remain GC-able. See docs/design/scan-output-backing-reuse.md.
// See docs/internals/scan-backing-pool-ownership.md for the design.
