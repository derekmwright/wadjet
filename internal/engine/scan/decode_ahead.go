...
// DecodeAheadIter delivers RowGroupIter's batches/errors/prunes in source order
// while workers decode ahead; see docs/design/scan-decode-pipelining.md.
// WindowBytes bounds decoded-undelivered estimates from projected uncompressed
// sizes, with error bounded by one group per worker. Always admit the delivery
// cursor's group, even oversized, to avoid deadlock.
// FileReader must stay read-only and ColumnPages must allocate independent readers.
// Start workers at FIRST Next, not Open; re-read filters on each assignment.
// Close stops assignment and JOINS every decode before the caller munmaps bytes.
// See docs/internals/scan-ordered-decode-ahead.md for the design.
