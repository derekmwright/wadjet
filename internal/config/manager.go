...
// ReloadWithReport applies the file and reports FileKeys intersected with
// !HotReloadable: startup-only keys remain at RUNNING values until restart.
// A file may legitimately carry next-start settings, so report them rather than
// use the PUT path's 409 refusal. Never silently imply those keys took effect.
// Do not diff against Load defaults: flag defaults differ (ADR-0029 decision 2)
// and would report keys the file never mentioned.
// See docs/internals/config-reload-ignored-key-report.md for the design.
