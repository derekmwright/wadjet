...
// genWindow includes comparison-chosen MIN/MAX values (#569).
// Use a single-column PK for unique running/sliding order; without it emit
// only whole-partition/OVER () shapes independent of tie order.
// Float SUM/AVG are non-exact (ADR-0013 §4); MIN/MAX/value functions are exact,
// and Opaque follows input kind rather than its display ordering.
// This generator uses bare plain-column PARTITION BY; #585's qualified/computed
// shapes have separate fixed coverage. Adding an arm changes draws for EVERY
// seed; keep resulting findings visible (#593, #594), not hidden by disabling it.
// See docs/internals/shapegen-deterministic-window-queries.md for the design.
