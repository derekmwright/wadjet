...
func TypeMatrix() *Schema {
	...
	// typematrix.Nested remains absent: containers are not generated directly and
	// star projections would add a separate nested read path. The historical process
	// killer is fixed; restoring this table and its id/g edges is a coverage change.
	// ROW field paths remain independently excluded (#568): Gen.name may qualify
	// them into unsupported three-part identity. Support needs a never-qualify
	// column rule or appropriate parser/resolver identity support.
	// Keep fixed typematrix rowfield_* and TestRowFieldPathCarriesTheFieldsDeclaredType
	// coverage; TestTypeMatrixNoProcessKillers covers process survival.
	// See docs/internals/shapegen-nested-schema-coverage-boundary.md for the design.
