...
// Generated comparisons declare SQL's determinism boundary (#288 follow-on):
// no ORDER BY compares multisets; total ORDER BY compares row sequence;
// possible ties compare multiset AND OrderKeys sequence, allowing tie reordering.
// Bare LIMIT compares counts only because selected rows are unspecified.
// CheckOrder independently validates returned keys are sorted, catching the
// same dropped ordering in both engines without a second reference.
// See docs/internals/oracle-query-determinism-modes.md for the design.
