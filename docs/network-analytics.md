...
Except for the coverage gaps listed below, that holds in **every position and on every plan**:
a `SELECT` item, a `WHERE`
...
those positions, and for one nested inside another subquery, however deep. For these covered calls,
a misspelling is refused before any stage runs, regardless of the data or
