...
| [0012](0012-sql-semantics-authority.md) | PostgreSQL is the SQL semantics authority; DuckDB is the performance goal and an oracle (amended 2026-09-24..26: one assignment table for every write, the temporal operators' types, the LATERAL refusals by property, the container closures) | Accepted |
...
| [0039](0039-a-table-function-in-from-is-a-relation.md) | A table function in FROM is a relation: the capability is authorized before the statement binds, so a signature-declared column list and a local file reader's are refused at plan time, and an input the planner does not read (`http(s)`, a database reader, a read-once input) at its first batch | Accepted |
