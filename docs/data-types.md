...
| `Port` | a DECIMAL number in 0–65535, with an optional sign and surrounding whitespace. A FRACTION is not a decimal number: the LITERAL `'2.5'` and a STRING COLUMN holding it are `22P02`, as `'2.5'::integer` is on the server — but a fraction reached through a string-typed EXPRESSION (`CAST(CONCAT('2','.5') AS PORT)`, `CAST(TRIM(s) AS PORT)`) still rounds to 3, which is ADR-0012 residual 4's open cell. int4's other spellings are NOT part of it: `'0x1bb'`, `'0o17'`, `'0b101'` and `'1_000'` are `22P02`, even though `'0x1bb'::integer` is 443. Service NAMES are not resolved either: `port_name()` is the function that names a port |
