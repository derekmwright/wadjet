...
//
// A star item now carries a (resolve, publish) pair (ADR-0026 §9), and the
// block publishes its visible list. Before that rule, it read the stream:
//
...
// for the statement's own SELECT list. The star expands the FROM arms in
// written order over those visible lists (ADR-0026 §9), independent of build side.
//
