...
StarSourceColumns supplies the list for a named relation or a single source;
a bare join star combines the FROM arms in written order (ADR-0026 §9). qualifier is
...
Each join arm or single source uses its published list, including for
