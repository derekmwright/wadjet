...
func TestArcRCRecursiveCTESeedTypeDecidesAgainstEveryTermType(t *testing.T) {
	...
	cells := []struct{ seed, term, sql, pg, want, why string }{
		...
		{"lit_dec", "float4", `WITH RECURSIVE r(n, k) AS (SELECT 1.5, 1 UNION ALL SELECT 2.5::real, k+1 FROM r WHERE k < 2) SELECT n::text AS n FROM r ORDER BY k`, "ERR 42804", "1.5;2.5", "superset: a fractional literal seed is double precision here, numeric there"},
		{"lit_dec", "float8", `WITH RECURSIVE r(n, k) AS (SELECT 1.5, 1 UNION ALL SELECT 2.5::float8, k+1 FROM r WHERE k < 2) SELECT n::text AS n FROM r ORDER BY k`, "ERR 42804", "1.5;2.5", "superset: a fractional literal seed is double precision here, numeric there"},
