...
func TestArcLTACorrelatedBodyIsEvaluatedPerOuterRow(t *testing.T) {
	...
	cases := []struct{ name, sql, want string }{
		...
		{"EXISTS over an ungrouped aggregate is true for every outer row",
			...
			"1;2;3;4;5"},
		// ROUND 2 (Codex review): the bound composes with the body, exactly.
		{"an ungrouped aggregate under LIMIT 0 yields no row on the INNER spelling",
			`SELECT o.id AS a, s.v AS v FROM lt_o o JOIN LATERAL (SELECT COUNT(*) AS v FROM lt_i i WHERE i.k = o.k LIMIT 0) s ON true ORDER BY a, v`,
			""},
		{"an ungrouped aggregate under OFFSET 1 yields no row on the INNER spelling",
			`SELECT o.id AS a, s.v AS v FROM lt_o o JOIN LATERAL (SELECT SUM(i.v) AS v FROM lt_i i WHERE i.k = o.k ORDER BY 1 OFFSET 1) s ON true ORDER BY a, v`,
			""},
		{"an ungrouped aggregate under LIMIT 0 pads on the LEFT spelling",
			`SELECT o.id AS a, s.v AS v FROM lt_o o LEFT JOIN LATERAL (SELECT COUNT(*) AS v FROM lt_i i WHERE i.k = o.k LIMIT 0) s ON true ORDER BY a, v`,
			"1,NULL;2,NULL;3,NULL;4,NULL;5,NULL"},
		{"ORDER BY a SELECT alias reads the alias's expression",
			`SELECT o.id AS a, s.v AS v FROM lt_o o JOIN LATERAL (SELECT i.v * -1 AS v FROM lt_i i WHERE i.k = o.k ORDER BY v, i.id LIMIT 1) s ON true ORDER BY a, v`,
			"1,-30;2,-30;3,-60"},
		{"ORDER BY a fresh SELECT alias",
			`SELECT o.id AS a, s.z AS z FROM lt_o o JOIN LATERAL (SELECT i.v * -1 AS z FROM lt_i i WHERE i.k = o.k ORDER BY z, i.id LIMIT 1) s ON true ORDER BY a, z`,
			"1,-30;2,-30;3,-60"},
		{"LIMIT max int64 OFFSET 1 does not wrap",
			`SELECT o.id AS a, s.v AS v FROM lt_o o JOIN LATERAL (SELECT i.v AS v FROM lt_i i WHERE i.k = o.k ORDER BY i.v, i.id LIMIT 9223372036854775807 OFFSET 1) s ON true ORDER BY a, v`,
			"1,10;1,30;2,10;2,30;3,40;3,60"},
		{"LIMIT 0 under an inequality correlation keeps the body's own empty bound",
			`SELECT o.id AS a, s.v AS v FROM lt_o o JOIN LATERAL (SELECT i.v AS v FROM lt_i i WHERE i.v > o.total LIMIT 0) s ON true ORDER BY a, v`,
			""},
		{"DISTINCT over exactly the key under LIMIT 1 is the identity",
			`SELECT o.id AS a, s.v AS v FROM lt_o o JOIN LATERAL (SELECT DISTINCT i.k AS v FROM lt_i i WHERE i.k = o.k ORDER BY i.k LIMIT 1) s ON true ORDER BY a, v`,
			"1,1;2,1;3,2"},
	...
	refusals := []struct{ name, sql, class, base string }{
		...
		{"a bound over the body's own QUALIFY",
			`SELECT o.id AS a, s.v AS v FROM lt_o o JOIN LATERAL (SELECT i.v AS v FROM lt_i i WHERE i.k = o.k QUALIFY ROW_NUMBER() OVER (PARTITION BY i.k ORDER BY i.v, i.id) > 1 ORDER BY i.v, i.id LIMIT 1) s ON true ORDER BY a, v`,
			"cannot apply that bound per outer row", "zero rows at b1e67749 (rn > 1 AND rn <= 1) for PostgreSQL's 3"},
